...
- `api/claims.js` is a Vercel serverless function; claims persist in Vercel Blob storage (`claims.json`) via `put`/`list`, read on every request (no caching). It also emails the couple on every claim and unclaim. The provider is whichever is configured, in order: direct SMTP (`SMTP_USER`/`SMTP_PASS`, e.g. a Gmail app password), Brevo (`BREVO_API_KEY`), then Resend (`RESEND_API_KEY`), all sharing `CLAIM_EMAIL_FROM`/`CLAIM_EMAIL_TO`. The couple owns no domain, which rules Resend out on its own — it only delivers to the account owner without a verified domain — so the fallback chain exists to keep a no-domain path open rather than to support providers for their own sake. They also have no access to a Google app password, so the primary notification route is a Google Sheet instead: each claim POSTs a row to an Apps Script web app (`SHEETS_WEBHOOK_URL`, optional `SHEETS_WEBHOOK_TOKEN`), and Sheets' own notification rules do the notifying. Its only credential is a URL, which is the point. Email and sheet are independent channels — one failing must not stop the other, and neither may fail a claim. Email is strictly best-effort: a missing config or a Resend error is logged and swallowed, never surfaced to the guest, because a claim that was persisted must not look like a failure.
